...
class RBAC {
    ...
    isAdmin() {
        // Accept both 'administrator' and 'admin' for backward compatibility
    ...
    applyPermissions() {
        ...
        // Add role badge and approval badge
        this.addRoleBadge();
        if (this.isAdmin()) {
            this.addApprovalBadge();
    ...
    // Add approval badge for admin
    addApprovalBadge() {
        const pendingCount = this.getPendingCount();
        if (pendingCount === 0) return;

        // Add badge to notifications or create approval menu item
        const notifLink = document.querySelector('a[href="notifications.html"]');
        if (notifLink && !document.getElementById('approvalBadge')) {
            const badge = document.createElement('span');
            badge.id = 'approvalBadge';
            badge.className = 'notification-badge';
            badge.style.cssText = 'background: #ff0055; margin-left: 8px;';
