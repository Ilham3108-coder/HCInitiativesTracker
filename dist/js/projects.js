...
window.openModal = function (initId = null) {
    ...
    const entityText = document.getElementById('projectEntityText');
    ...
    const entityNames = {
        ...
        'HC Center': 'HC Center',
        'HC Business Partner': 'HC Business Partner',
    ...
    if (entitySelect && entitySelect.options.length === 0) {
        ...
        const orderedKeys = [
            ...
            'HC Center', 'HC Business Partner', 'Talent Management', 'Organization Development' // Others
        ];
    ...
    if (initId) {
        // Edit Mode - Use allInitiatives to allow admin to edit any initiative
        const init = allInitiatives.find(i => i.id === initId);
        if (!init) return;

        // CRITICAL: Set the hidden ID field so submit handler knows it's an update
        document.getElementById('projectId').value = initId;

        modalTitle.innerText = "Edit Initiative";
        ...
        if (pDueDate) pDueDate.value = init.dueDate;
        ...
        entityText.style.display = 'none';

        // Render sub-initiatives
        if (init.subInitiatives && init.subInitiatives.length > 0) {
            init.subInitiatives.forEach(sub => addSubInitiativeField(sub.name, sub.weight, sub.dueDate, sub.progress));
        }
    } else {
        // Create Mode
        modalTitle.innerText = "Insert Initiative";
    ...
    if (pStatus) {
        ...
        // Similarly for Urgency (Only if it's a dropdown)
        const pUrgency = document.getElementById('pUrgency');
        if (pUrgency && pUrgency.tagName === 'SELECT') {
            Array.from(pUrgency.options).forEach(opt => {
                opt.style.background = '#1a1a2e';
                opt.style.color = 'white';
            });
        }
    ...
    if (pDueDate && pUrgency) {
        ...
        const freshDueDate = document.getElementById('pDueDate');
...
window.handleProjectSubmit = function (e) {
    ...
    if (id) {
        // Update Existing
        project = allInitiatives.find(i => i.id == id);
        if (project) {
            // Check if user is regular user - if so, request approval for update
            if (window.rbac && window.rbac.isUser()) {
                ...
                // Calculate proposed progress for the pending update
                ...
                if (keyActivities.length > 0) {
                    ...
                    if (totalWeight > 0) {
                        proposedProgress = Math.round(weightedProgressSum / 100);
                    } else {
                        const totalP = keyActivities.reduce((sum, s) => sum + (s.progress || 0), 0);
                        proposedProgress = Math.round(totalP / keyActivities.length);
                    }
                } else {
                ...
                if (window.addNotification) {
                    window.addNotification('info', `Update request sent for: "${name}"`, currentUsername);
                    // Notify Admin
                    window.addNotification('warning', `Update request from ${currentUsername} for initiative: "${name}"`, 'admin');
                }

                alert("✅ Request to Updated Initiatives has been sent successfully to the Administrator for approval.");
                ...
                project.cost = cost;
                project.subInitiatives = keyActivities;

                // Recalculate main progress based on sub-activities
                if (keyActivities.length > 0) {
                ...
                // Notification
                if (window.addNotification) {
                    window.addNotification('info', `Updated initiative: "${name}"`, currentUsername);
                }

                // Broadcast to all users if admin edited someone else's initiative
                if (window.rbac?.isAdmin() && project.createdBy !== currentUsername && window.notificationManager) {
                    window.notificationManager.broadcastToAllUsers(
                        'info',
                        `Administrator updated initiative: "${name}"`,
                        currentUsername
                    );
                }
            }
        ...
        project = {
            ...
            cost: cost,
            status: status,
            progress: keyActivities.length > 0 ? 0 : progress,
            subInitiatives: keyActivities,
            createdBy: currentUsername,
            // Users -> Pending, Admins -> Approved
            approvalStatus: (window.rbac && window.rbac.isUser()) ? 'pending' : 'approved'
        };

        allInitiatives.push(project);

        // Notification
        if (window.addNotification) {
            window.addNotification('success', `Created new initiative: "${name}"`, currentUsername);
        }

        // Popup for regular users
        if (window.rbac && window.rbac.isUser()) {
            setTimeout(() => {
                alert("✅ Initiative has been sent successfully to the Administrator for approval.");
            }, 100);
        }
    }
    ...
    // Force refresh ID to prevent duplicate adds if user clicks save again quickly
...
window.toggleActions = function (event, id) {
    ...
    // Create Update Initiatives option
    const editOption = document.createElement('div');
    editOption.style.padding = '10px 20px';
    editOption.style.cursor = 'pointer';
    editOption.style.color = 'white';
    editOption.style.borderBottom = '1px solid #333';
    editOption.style.fontSize = '13px';
    editOption.textContent = 'Update Initiatives';
    editOption.addEventListener('click', (e) => {
        e.stopPropagation();
        console.log('Update Initiatives clicked for initiative ID:', id);
        console.log('openModal function exists:', typeof window.openModal);
        window.openModal(id);
        menu.remove();
    });
    ...
    deleteOption.style.padding = '10px 20px';
    deleteOption.style.cursor = 'pointer';
    deleteOption.style.color = '#ff0055';
    ...
    // Find the initiative header to append menu to
    let target = event.target;
    // If clicking on SVG or path, go up to find the icon or header
    while (target && !target.classList.contains('initiative-header')) {
...
window.deleteProject = function (id) {
    ...
    if (window.rbac && !window.rbac.canDelete(initiative)) {
        alert('❌ You cannot delete this initiative. Only pending or rejected initiatives can be deleted.');
        return;
    }
